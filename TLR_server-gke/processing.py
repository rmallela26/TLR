...
# bucket_name = "rish-bucket"
# bucket = storage_client.get_bucket(bucket_name)
# blob = bucket.blob('updates.txt')
...
#     prediction = 1 #############PREDICTIONS ARE BETTER WITHOUT THIS, NOW DON'T NEED PAUSE BUTTON
else:
    prediction = 0
...
f.write(requestId + "\n")
# with blob.open("w") as f:
#     f.write("interpolation done\ncounts done\ngot motion feature\ngot probabilties\npost processing done\ndone writing to answer\n")

f = open(f'{uuid}_labels.txt', "a") #keep track of the label for each epoch
f.write(l1)
